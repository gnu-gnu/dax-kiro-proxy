...
type batchHTTPGate struct {
	*Manager
	...
	observe        func(batchHTTPProcess) error
	beforeRecovery func(batchHTTPProcess)
}
...
func buildBatchHTTPPeers(t *testing.T, ctx context.Context) (string, string, string) {
	t.Helper()
	root := t.TempDir()
	...
	return root, fake, executable
}

func runBatchHTTPWave(t *testing.T, ctx context.Context, g *batchHTTPGate, server *gateway.Server, client *http.Client, token string, wave int) [3]batchHTTPProcess {
	t.Helper()
	m := g.Manager
	streaming := wave%2 == 1
	...
	post := func(state batchHTTPState, status int) (anthropic.Response, error) {
		result, err := batchHTTPPost(ctx, client, server.URL(), token, model, state, streaming, status)
		if err != nil {
			return result, err
		}
	...
	if g.beforeRecovery != nil {
		g.beforeRecovery(firstB)
	}
	...
	fresh := g.owners[states[1].id][1]
	if observed.PID != fresh.pid || observed.RelayPID != fresh.peer || observed.RelayConfig != fresh.config {
		t.Fatal("recovery response did not match its new binding")
	}
	return [3]batchHTTPProcess{firstA, firstB, fresh}
}
