...
- D136 on d136-prepared-batch-churn (base c9f7ebc) retains one gateway/manager/pool across
  ...
  preparation. The final 216.765-second active loop passes under race (package 221.297s).
  After four warm-up waves, descriptors/goroutines stay 22/30; post-GC heap peaks at
  1,083,952 bytes from 689,432. Final values are 5/2/879,832. Real retained-resource controls
...
- D131 on d131-account-check (base 14a0478) fixes account-command deadlines and
  ...
  output on deadline/nonzero exit, stop both inspection and run before policy and join the owned
