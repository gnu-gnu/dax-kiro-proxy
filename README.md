...
Phase 7 has frozen dependency inventories and retained scoped notices. For the D110–D112
client-admission development artifact, run `python3 tools/verify_dependency_inventory.py --gomodcache .cache/gomod
--snapshot client-version --binary dist/dax-kiro-proxy`. The default `development` snapshot identifies
...
identifies D108 and `tool-images` identifies D109. Those historical snapshots do not match this
rebuild and its changed production source. These offline byte checks do not grant release license
