...
Phase 7 has frozen dependency inventories and retained scoped notices. For the D115 onboarding
development artifact, run `python3 tools/verify_dependency_inventory.py --gomodcache .cache/gomod
--snapshot onboarding --binary dist/dax-kiro-proxy`. The default `development` snapshot identifies
...
`measured-client` identifies D113 and `measured-kiro` identifies D114. Those historical snapshots do
not match this rebuild and its changed production source. These offline byte checks do not grant
release license clearance; see DEPENDENCY_REVIEW.md for resource differences and remaining
packaging/rights work.
